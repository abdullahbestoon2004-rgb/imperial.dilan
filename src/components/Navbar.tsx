    ...
    return items.map((item) => {
      ...
      if (!hasChildren) {
        return (
          <NavAnchor
            ...
            className={`block rounded-md px-2 py-2 font-['Inter'] text-[#171419] transition-colors hover:bg-[#1E2430]/10 hover:text-[#0B0A0D] ${depthPadding} ${
      ...
      return (
        <div key={key} className="space-y-1">
          <button
            type="button"
            className={`flex w-full items-center justify-between rounded-md px-2 py-2 text-left font-['Inter'] text-[#171419] transition-colors hover:bg-[#1E2430]/10 hover:text-[#0B0A0D] ${depthPadding} ${
              depth === 0 ? 'text-sm' : 'text-[13px]'
            }`}
          ...
          >
            <div className="overflow-hidden border-l border-[#6E5A44]/35">
  ...
  return (
    <nav
      className={`fixed left-0 right-0 top-0 z-50 transition-all duration-300 ${
        scrolled || mobileOpen
          ? 'bg-[#171419]/78 shadow-[0_12px_35px_rgba(15,7,1,0.35)] backdrop-blur-md'
          : 'bg-[#171419]/45 backdrop-blur-sm'
      }`}
    >
      <div className="mx-auto max-w-[1700px] px-4 md:px-6">
        <div className="flex h-[78px] items-center md:h-[84px]">
              ...
              {NAV_ITEMS.map((item) => {
                ...
                if (!hasDropdown) {
                  return (
                    <NavAnchor
                      key={item.label}
                      item={item}
                      className="font-['Inter'] text-[11px] tracking-wide text-[#D9CFBF] transition-colors hover:text-[#C6A66B]"
                    />
                  );
                }
                ...
                return (
                  ...
                  >
                    <button
                      type="button"
                      className="flex items-center gap-1 font-['Inter'] text-[11px] tracking-wide text-[#D9CFBF] transition-colors hover:text-[#C6A66B]"
                    ...
                    <div
                      onMouseEnter={clearCloseTimer}
                      onMouseLeave={scheduleDropdownClose}
                      className={`absolute left-1/2 top-full z-50 mt-0 w-[min(88vw,900px)] -translate-x-1/2 rounded-2xl border border-[#6E5A44]/35 bg-[#D9CFBF] p-6 shadow-[0_20px_55px_rgba(15,7,1,0.35)] transition-all duration-300 ${
                      ...
                      <div className={`grid grid-cols-1 gap-x-10 gap-y-6 ${columnsClass}`}>
                        {item.children?.map((group) => (
                          <div key={group.label} className="space-y-2">
                            <NavAnchor
                              item={group}
                              className="block font-['Playfair_Display'] text-[18px] text-[#171419] transition-colors hover:text-[#C6A66B]"
                            />

                            {group.children?.length ? (
                              <div className="space-y-1 border-l border-[#6E5A44]/35 pl-3">
                                {group.children.map((child) => (
                                  <NavAnchor
                                    key={child.label}
                                    item={child}
                                    className="block font-['Inter'] text-sm text-[#6E5A44] transition-colors hover:text-[#171419]"
                                  />
                                ))}
                              </div>
                            ) : null}
          ...
          <div className="hidden w-[180px] shrink-0 justify-end gap-4 lg:flex">
            <button className="text-[#D9CFBF] transition-colors hover:text-[#C6A66B]" aria-label="Search">
              <Search size={19} />
            </button>
            <button className="text-[#D9CFBF] transition-colors hover:text-[#C6A66B]" aria-label="Account">
              <User size={19} />
            </button>
            <button className="text-[#D9CFBF] transition-colors hover:text-[#C6A66B]" aria-label="Wishlist">
              <Heart size={19} />
            </button>
            <button className="text-[#D9CFBF] transition-colors hover:text-[#C6A66B]" aria-label="Cart">
          ...
          <div className="ml-auto flex items-center gap-3 lg:hidden">
            ...
            <button
              className="text-[#D9CFBF] transition-colors hover:text-[#C6A66B]"
      ...
      >
        <div className="border-t border-[#6E5A44]/35 bg-[#D9CFBF]">
